...
import { MediaGalleryItem, SpecTableItem } from '@/types/product'; // SpecTableItem 추가
...
const ProductDetail: React.FC<ProductDetailClientProps> = ({
    ...
    approvalNumber,
    cautions = [],
    // pageBackgroundImage, // 사용되지 않으므로 주석 처리 또는 제거 가능
}) => {
    const sectionRefs = {
    ...
    const currentProductName = initialProductName || "수직강하식 구조대";
    const currentDescription = initialDescription || "화재시 또는 재난상황 발생시 이동설치가 가능한 구조로 가볍고 신속한 설치가 가능한 구조의 인명구조 장비입니다.";
    const currentApprovalNumber = approvalNumber || "형식승인번호 : 구23-3";
    ...
    const displayFeaturesToRender = useMemo(() =>
        features && features.length > 0 ? features.map(({ icon, ...rest }) => rest) :
            [
                { title: "신속한 설치", description: "이동 및 설치가 용이하여 긴급 상황 발생 시 빠르게 대응 가능합니다." },
                { title: "경량 구조", description: "가벼운 소재를 사용하여 휴대 및 조작이 간편합니다." },
                { title: "안전한 강하", description: "일정한 속도로 안전하게 강하할 수 있도록 설계되었습니다." },
                { title: "다양한 적용", description: "건물 구조 및 높이에 따라 다양한 길이로 제공됩니다." },
            ],
        [features]);

    const defaultSpecTable: SpecTableItem[] = [
        { title: "보관규격 가로×세로×높이(mm)", "value": "880 X 500 X 960 / 880 × 450 × 1,300" }, // 두 규격을 하나로 합치거나, 모델 구분 필요
        { title: "활강포 길이 (m)", "value": "4.5 ~ 70 m" },
    ...
    const scrollToSection = (sectionId: SectionId) => {
        ...
        // 필요하다면 IntersectionObserver 등으로 스크롤 완료 감지 후 isScrolling = false 처리
        ...
        <div className="relative min-h-screen text-white">
            <div className="fixed inset-0 z-0">
                <Image
                    ...
                    priority
                    sizes="100vw" // 화면 전체를 차지하므로 100vw
                    quality={85} // 품질 약간 조정 (기본값 75)
                />
            ...
            <div className="relative z-10">
                ...
                <div className="relative bg-gradient-to-b from-black/[.70] via-black/50 to-transparent">
                    <section
                        ...
                        id="overview"
                        className="min-h-screen flex flex-col justify-center relative py-20 scroll-mt-20" // scroll-mt-20 추가 (헤더 높이 고려)
                    >
                        <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-stretch">
                                <div className="flex flex-col h-full">
                                    ...
                                    >
                                        <Image
                                            src={mainImage || "/images/products/Vertical-Rescue-Equipment/thumbnail.jpg"}
                                            alt={`${currentProductName} 제품 이미지`}
                                            fill
                                            className="object-cover rounded-2xl"
                                            sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 512px" // 반응형 이미지 크기 제공
                                            quality={80}
                                        />
                                    </div>
                        ...
                        <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">
                                    수직강하식 구조대의 상세한 기술 사양 정보를 확인하세요.
                                </p>
                            </div>
                            <div className="bg-black/50 backdrop-blur-md border border-white/15 rounded-xl shadow-2xl overflow-hidden transform hover:scale-[1.01] transition-transform duration-300">
                                ...
                                <div className="p-2 md:p-0">
                                    {currentSpecTable && Array.isArray(currentSpecTable) && currentSpecTable.length > 0 ? (
                                        <ModelSpecTable specTable={currentSpecTable} className="w-full bg-transparent" />
                                    ) : (
                    ...
                    >
                        <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <div className="w-24 h-1 mx-auto bg-white/50 rounded-full mb-6 animate-pulse" style={{ animationDuration: '3s' }}></div>
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">
                                    다양한 건물 및 비상 상황에 설치되어 활용되는 수직강하식 구조대의 실제 모습을 확인하세요.
                                </p>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                                {
                                    imageItems.length > 1 ? (
                                        [
                                            {
                                                imgSrc: imageItems[0]?.src || "/images/products/Vertical-Rescue-Equipment/gallery/sample1.jpg", // 기본 이미지 경로 수정
                                                title: "수직강하식 구조대 설치 모습",
                                                description: "건물 외벽에 설치된 수직강하식 구조대의 실제 설치 모습입니다."
                                            },
                                            {
                                                imgSrc: imageItems[1]?.src || "/images/products/Vertical-Rescue-Equipment/gallery/sample2.jpg", // 기본 이미지 경로 수정
                                                title: "수직강하식 구조대 사용 시연",
                                                description: "실제 피난 상황을 가정한 수직강하식 구조대 사용 시연 장면입니다."
                                            }
                                        ].map((item, index) => (
                    ...
                    >
                        <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <div className="w-24 h-1 mx-auto bg-white/50 rounded-full mb-6 animate-pulse" style={{ animationDuration: '3s' }}></div>
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">
                                    수직강하식 구조대의 다양한 이미지와 작동 영상을 통해 제품을 자세히 살펴보세요.
                                </p>
                                    ...
                                    {activeMediaTab === 'images' && (
                                        imageItems.length > 0 ? (
                                            <div className="animate-fadeIn">
                                                {/* ProductImageGallery에 sizes prop 전달 고려 */}
                        ...
                        __html: JSON.stringify({
                            ...
                            ...(currentSpecTable && currentSpecTable.length > 0 && {
                                "additionalProperty": currentSpecTable.map(spec => ({
                                    "@type": "PropertyValue",
                                    "name": spec.title,
                                    // `spec.value`가 있는 경우 우선 사용, 그렇지 않으면 기존 로직 (첫 번째 값) 사용
                                    "value": spec.value || spec[Object.keys(spec).find(key => key !== 'title') || ""]
                                }))
                            }),
                            "material": "특수 방염 처리된 활강포",
                            ...(currentApprovalNumber && {
                                "certification": {
                                    "@type": "Certification",
                                    "name": "한국소방산업기술원 형식승인",
                                    // 형식승인번호에서 실제 번호만 추출 (예: "구23-3")
