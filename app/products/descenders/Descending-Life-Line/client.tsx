...
import {
    ...
    AlertTriangle, ChevronDown, CheckCircle2, ExternalLink, Download, ChevronRight, Award, Users // Users 아이콘 추가 (완강기 특성)
} from 'lucide-react';
...
const ProductDetail: React.FC<ProductDetailClientProps> = ({
    ...
    approvalNumber,
    cautions = [],
    // pageBackgroundImage, // 완강기 페이지에서는 Air-Slide와 달리 이 prop을 사용하지 않을 수 있음.
}) => {
    // 섹션 레퍼런스 및 활성 섹션 추적
    const sectionRefs = {
    ...
    const [sideNavOffset, setSideNavOffset] = useState(0); // 이 변수는 현재 코드에서 사용되지 않지만, 추후 활용 가능성 있음
    const [windowHeight, setWindowHeight] = useState(0); // 이 변수는 현재 코드에서 사용되지 않지만, 추후 활용 가능성 있음
    ...
    // 윈도우 높이 설정 (스크롤 이벤트 내에서 window.innerHeight를 직접 사용하므로 불필요할 수 있음)
    useEffect(() => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            // 히어로 섹션을 지나면 내비게이션 표시 (window.innerHeight 직접 사용)
    ...
    }, [isScrolling, activeSection]); // windowHeight 종속성 제거
    ...
    const scrollToSection = (sectionId: SectionId) => {
        ...
        const targetPosition = targetSection.offsetTop; // 오프셋 0으로 고정 또는 필요시 동적 계산
    ...
    return (
        <div className="relative min-h-screen text-white">
            {/* 페이지 전체 배경 - 오버레이 밝기 조절 */}
            <div className="fixed inset-0 z-0">
                <Image
                    src={"/images/products/Descending-Life-Line/main/visual.jpg"}
                    alt={`${currentProductName} 배경`}
                    fill
            ...
            <div className="relative z-10">
                    ...
                    <div className="max-w-7xl w-full mx-auto px-6 md:px-8 lg:px-12 pt-24 flex flex-col items-center justify-center h-full relative z-10">
                        <div className="text-center mb-8">
                            ...
                            <p className="text-lg text-gray-300 mb-2 leading-relaxed">
                                사용자의 체중에 따라 자동으로 안전하게 하강하는 피난 장비입니다.
                            </p>
                        </div>
                    </div>
                    {/* 중앙 하단에 스크롤 유도 아이콘 */}
                ...
                {/* 히어로 이후 모든 콘텐츠 섹션을 감싸는 단일 배경 컨테이너 - 외곽 투명 처리 */}
                <div className="relative bg-gradient-to-b from-transparent via-black/50 to-transparent pt-1">
                    {/* Overview 섹션 - 개별 배경 제거 */}
                    <section
                    ...
                    >
                        <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white font-serif" style={{ fontFamily: 'var(--font-playfair)' }}>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-stretch">
                                ...
                                <div className="flex flex-col h-full">
                                    <div className="relative aspect-[4/3] w-full max-w-xl mx-auto flex-shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-black/40" style={{ minWidth: '220px', minHeight: '180px' }}>
                                        <Image
                                            src={mainImage || "/images/products/Descending-Life-Line/thumbnail.jpg"}
                    ...
                    {/* TechnicalSpecifications 섹션 - 개별 배경 제거 */}
                    <section ref={sectionRefs.technicalSpecifications} className="min-h-screen flex flex-col justify-center relative py-20">
                        <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <div className="w-24 h-1 mx-auto bg-white/50 rounded-full mb-6 animate-pulse" style={{ animationDuration: '3s' }}></div>
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">{currentProductName}의 상세한 기술 사양 및 구성품을 확인하세요.</p>
                            </div>
                            <div className="bg-black/50 backdrop-blur-md border border-white/15 rounded-xl shadow-2xl overflow-hidden transform hover:scale-[1.01] transition-transform duration-300">
                                <div className="p-6 md:p-8 border-b border-white/10">
                                    <h3 className="text-xl lg:text-2xl font-bold text-white font-serif flex items-center" style={{ fontFamily: 'var(--font-playfair)' }}>
                                        <Package className="h-5 w-5 lg:h-6 lg:w-6 mr-3 text-sky-400 flex-shrink-0" />제품 상세 규격
                                    </h3>
                                </div>
                                <div className="p-2 md:p-0">
                                    {specTable && Array.isArray(specTable) && specTable.length > 0 ? <ModelSpecTable specTable={specTable} className="w-full bg-transparent" /> : (
                                            ...
                                            <div className="mt-6 max-w-md mx-auto text-left">
                                                <table className="w-full text-sm">
                                                    <tbody className="divide-y divide-white/10">
                                                        {[
                                                            { label: "사용 인원", value: "1인 연속 교대 사용" },
                                                            { label: "로프 재질", value: "항공용 특수 와이어로프 (PVC코팅)" },
                                                            { label: "하강 속도", value: "사용자 체중에 따라 자동 조절 (평균 1m/sec)" },
                                                            { label: "설치 길이", value: "현장 맞춤 제작" },
                                                            { label: "주요 구성품", value: "조속기, 로프, 안전벨트, 보관함, 지지대 등" },
                                                        ].map(item => (<tr key={item.label} className="hover:bg-white/5"><td className="py-3 px-4 font-medium text-white/90">{item.label}</td><td className="py-3 px-4 text-gray-300">{item.value}</td></tr>))}
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
                    ...
                    {/* CaseStudies 섹션 - 개별 배경 제거 */}
                    <section ref={sectionRefs.caseStudies} className="min-h-screen flex flex-col justify-center relative py-20">
                        <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <div className="w-24 h-1 mx-auto bg-white/50 rounded-full mb-6 animate-pulse" style={{ animationDuration: '3s' }}></div>
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">다양한 건물 및 시설에 설치된 {currentProductName}의 실제 활용 모습을 확인하세요.</p>
                            </div>
                            {/* 완강기 설치 사례 이미지 경로 및 설명으로 변경 필요 */}
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                                {[
                                    { imgSrc: "/images/products/Descending-Life-Line/gallery/case1.jpg", title: "아파트 설치 사례", description: "고층 아파트 각 세대별 비상 탈출용으로 설치된 완강기." },
                                    { imgSrc: "/images/products/Descending-Life-Line/gallery/case2.jpg", title: "학교/기숙사 적용", description: "학생들의 안전을 위해 학교 및 기숙사에 설치된 모습." },
                                    { imgSrc: "/images/products/Descending-Life-Line/gallery/case3.jpg", title: "다중이용시설 설치", description: "병원, 호텔 등 다중이용시설의 비상 대피 계획에 포함된 완강기." }
                                ].map((item, index) => (
                            ...
                            <p className="text-center mt-10 text-gray-500 text-sm">* 위 이미지는 이해를 돕기 위한 예시이며, 실제 설치 환경에 따라 달라질 수 있습니다. (실제 완강기 이미지로 교체 필요)</p>
                        </div>
                    </section>

                    {/* Documents 섹션 - 개별 배경 제거 */}
                    <section ref={sectionRefs.documents} className="min-h-screen flex flex-col justify-center relative py-20">
                        <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-20">
                                {/* CTA 카드 내용 등은 제품 공통으로 사용 가능 */}
                                <div className="max-w-3xl mx-auto bg-gradient-to-br from-[#1f1f22] to-black backdrop-blur-lg border border-white/15 rounded-2xl p-8 md:p-10 shadow-xl relative overflow-hidden group transform hover:scale-105 transition-transform duration-300">
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                            ...
                                            ><MessageCircle className="mr-2 h-5 w-5" />온라인 문의하기</Link>
                                            <Button variant="outline" className="px-6 py-3 border-white/30 text-gray-200 hover:text-white hover:bg-white/10 hover:border-white/50 backdrop-blur-sm rounded-md transition-all duration-300 text-base font-medium transform hover:-translate-y-0.5" onClick={() => window.history.back()}><ArrowLeft className="mr-2 h-5 w-5" />모든 제품 보기</Button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="max-w-3xl mx-auto bg-black/50 backdrop-blur-md border border-white/15 rounded-xl p-6 md:p-8 shadow-2xl transform hover:scale-[1.01] transition-transform duration-300">
                                {documents && documents.length > 0 ? (
                                    <div className="space-y-4">{documents.map((doc, idx) => (<Link
                                        key={doc.id || idx}
                                    ...
                                    ><div className="w-10 h-10 sm:w-12 sm:h-12 rounded-md bg-black/60 border border-white/20 flex items-center justify-center mr-4 group-hover:scale-105 group-hover:border-red-400/50 transition-all duration-300 flex-shrink-0"><FileTextIcon className="w-5 h-5 sm:w-6 sm:h-6 text-red-400/80 group-hover:text-red-400 transition-colors" /></div><div className="flex-1 min-w-0"><h4 className="text-base sm:text-lg lg:text-xl font-medium text-white truncate group-hover:text-red-300 transition-colors">{doc.nameKo || doc.name}</h4><p className="text-xs sm:text-sm lg:text-base text-gray-400">{(doc.type || '파일').toUpperCase()} 형식{doc.fileSize && <span className="ml-2 text-gray-500">({doc.fileSize})</span>}</p></div><div className="ml-3 p-2 rounded-full bg-black/30 group-hover:bg-red-600/30 transition-all duration-300 opacity-80 group-hover:opacity-100"><Download className="h-4 w-4 sm:h-5 sm:h-5 text-white/70 group-hover:text-white transition-colors" /></div></Link>))}</div>
                                ) : (<div className="text-center py-16"><div className="relative w-20 h-20 mx-auto mb-6"><div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent rounded-full animate-pulse"></div><div className="absolute inset-2 bg-gradient-to-br from-white/10 to-transparent rounded-full animate-pulse" style={{ animationDelay: '300ms' }}></div><FileTextIcon className="h-10 w-10 text-gray-600 absolute inset-0 m-auto opacity-70" /></div><p className="text-gray-400 text-lg lg:text-xl">등록된 문서가 없습니다.</p><p className="text-gray-500 text-sm lg:text-base mt-2">필요한 자료는 문의해 주시면 신속히 제공해 드리겠습니다.</p></div>)}
                            </div>
                        </div>
                    </section>

                    {/* Gallery 섹션 - 개별 배경 제거 */}
                    <section ref={sectionRefs.gallery} className="min-h-screen flex flex-col justify-center relative py-20">
                    ...
                    __html: JSON.stringify({
                        ...
                        ...(features && features.length > 0 && {
                            "additionalProperty": features.map(feature => ({
                                "@type": "PropertyValue", "name": feature.title, "value": feature.description
                            }))
                        }),
                    })
                }} />
