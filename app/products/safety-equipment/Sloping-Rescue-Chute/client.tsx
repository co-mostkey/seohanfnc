...
const ProductDetail: React.FC<ProductDetailClientProps> = ({
    ...
    const currentProductName = initialProductName || "경사강하식 구조대";
    const currentDescription = initialDescription || "화재시 또는 재난상황 발생시 자루 형태의 포지를 이용하여 45도 각도의 경사면을 만들어 미끄럼 타듯 내려가는 구조로, 짧은 시간에 많은 인원이 탈출 할 수 있는 구조 장비입니다.";
    const currentApprovalNumber = approvalNumber || "형식승인번호 : 구 04-2-1";
    ...
    const displayFeaturesToRender = useMemo(() =>
        features && features.length > 0 ? features.map(({ icon, ...rest }) => rest) :
            [
                { title: "다수인 신속 탈출", description: "자루 형태의 포지를 이용, 45도 경사면으로 미끄럼 타듯 내려와 짧은 시간에 많은 인원이 대피 가능합니다." },
                { title: "안전한 경사 구조", description: "제어된 각도의 경사면을 통해 사용자가 안전하게 지상으로 대피할 수 있도록 설계되었습니다." },
                { title: "간편한 사용법", description: "특별한 조작 없이 미끄럼틀처럼 간단하게 사용하여 신속한 탈출이 가능합니다." },
                { title: "공식 인증 제품", description: "한국소방산업기술원의 형식승인을 받은 안전한 제품입니다." },
            ],
        [features]);

    const defaultSpecTable: SpecTableItem[] = [
        { title: "구분", model_2_5: "2~5층형", model_6_10: "6~10층형" },
        { title: "보관규격 가로×세로×높이(mm)", model_2_5: "750 × 400 × 800", model_6_10: "750 × 500 × 1,000" },
        { title: "활강포 길이 (m)", model_2_5: "7 ~ 20 m", model_6_10: "21 ~ 60 m" },
        { title: "활강시간 (sec.)", model_2_5: "3 ~ 4 m/sec.", model_6_10: "3 ~ 4 m/sec." },
        { title: "활강간격 (sec.)", model_2_5: "4 ~ 5 sec.", model_6_10: "4 ~ 5 sec." },
        { title: "관련검사", model_2_5: "한국소방산업기술원 제품승인품", model_6_10: "한국소방산업기술원 제품승인품" },
    ];
    ...
    const currentCautions = cautions && cautions.length > 0 ? cautions : [
        ...
        "탈출전 지상에 반드시 설치를 위한 요원이 배치되어야 합니다.",
    ];
    ...
    return (
        <div className="relative min-h-screen text-white">
            <div className="fixed inset-0 z-0">
                <Image
                    src={mainImage || "/images/products/Sloping-Rescue-Chute/main/visual.jpg"}
            ...
            {showNav && (
                <div className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-md border-b border-gray-800/30 py-2 px-4 transition-all duration-300 shadow-lg animate-fadeIn">
                    <div className="max-w-7xl mx-auto flex items-center justify-between">
                        ...
                        <Link href={`/support/contact?product_name=${encodeURIComponent(currentProductName)}&product_id=${encodeURIComponent(productId)}`} className="flex items-center px-3 py-1 rounded-md bg-gradient-to-r from-red-700 to-red-600 hover:from-red-800 hover:to-red-700 text-white font-medium text-xs transition-all duration-300 shadow-md">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-stretch">
                                <div className="flex flex-col h-full">
                                    <div className="relative aspect-[4/3] w-full max-w-xl mx-auto flex-shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-black/40" style={{ minWidth: '220px', minHeight: '180px' }}>
                                        <Image src={mainImage || "/images/products/Sloping-Rescue-Chute/thumbnail.jpg"} alt={`${currentProductName} 제품 이미지`} fill className="object-cover rounded-2xl" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 512px" quality={80} />
                                    </div>
                    ...
                    <section ref={sectionRefs.technicalSpecifications} id="technicalSpecifications" className="min-h-screen flex flex-col justify-center relative py-20 scroll-mt-20">
                        <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <div className="w-24 h-1 mx-auto bg-white/50 rounded-full mb-6 animate-pulse" style={{ animationDuration: '3s' }}></div>
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">경사강하식 구조대의 상세한 기술 사양 정보를 확인하세요.</p>
                            </div>
                    ...
                    <section ref={sectionRefs.caseStudies} id="caseStudies" className="min-h-screen flex flex-col justify-center relative py-20 scroll-mt-20">
                        <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <div className="w-24 h-1 mx-auto bg-white/50 rounded-full mb-6 animate-pulse" style={{ animationDuration: '3s' }}></div>
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">다양한 건물 및 비상 상황에 설치되어 활용되는 경사강하식 구조대의 실제 모습을 확인하세요.</p>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                                {imageItems.length > 1 ? ([
                                    { imgSrc: imageItems[0]?.src || "/images/products/Sloping-Rescue-Chute/gallery/case1.jpg", title: "경사강하식 구조대 설치 모습", description: "건물 외벽 비상구에 설치된 경사강하식 구조대." },
                                    { imgSrc: imageItems[1]?.src || "/images/products/Sloping-Rescue-Chute/gallery/case2.jpg", title: "경사강하식 구조대 사용 훈련", description: "소방 훈련 중 경사강하식 구조대를 이용한 대피 훈련 모습." }
                                ].map((item, index) => (
                                ...
                                {documents && documents.length > 0 ? (
                                    <div className="space-y-4">
                                        {documents.map((doc, idx) => (
                                            <Link key={doc.id || idx} href={doc.path || doc.url || '#'} target="_blank" rel="noopener noreferrer" className="group flex items-center p-4 sm:p-5 bg-black/40 hover:bg-black/60 border border-white/10 hover:border-white/20 rounded-lg transition-all duration-300 transform hover:shadow-lg">
                    ...
                    <section ref={sectionRefs.gallery} id="gallery" className="min-h-screen flex flex-col justify-center relative py-20 scroll-mt-20">
                        <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 lg:px-8 w-full">
                            <div className="text-center mb-16">
                                ...
                                <div className="w-24 h-1 mx-auto bg-white/50 rounded-full mb-6 animate-pulse" style={{ animationDuration: '3s' }}></div>
                                <p className="text-gray-400 max-w-2xl mx-auto text-base lg:text-lg">경사강하식 구조대의 다양한 이미지와 작동 영상을 통해 제품을 자세히 살펴보세요.</p>
                        ...
                        __html: JSON.stringify({
                            ...
                            ...(currentSpecTable && currentSpecTable.length > 0 && {
                                "additionalProperty": currentSpecTable.map(spec => ({
                                    "@type": "PropertyValue",
                                    "name": spec.title,
                                    // 모델별 값이 있을 경우, 해당 모델 값들을 문자열로 합쳐서 제공하거나, 첫 번째 모델 값만 사용
                                    "value": spec.model_2_5 && spec.model_6_10 ? `2~5층형: ${spec.model_2_5}, 6~10층형: ${spec.model_6_10}` : (spec.model_2_5 || spec.model_6_10 || spec.value || 'N/A')
                                }))
                            }),
