...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        console.error('[GET /api/as/:id]', error);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!updated) {
            return NextResponse.json({ error: '수정할 글이 없거나 실패했습니다.' }, { status: 404 });
        }

        return NextResponse.json({ message: '수정 완료', data: updated });
    } catch (error) {
        console.error('[PATCH /api/as/:id]', error);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!success) {
            return NextResponse.json({ error: '삭제할 글이 없습니다.' }, { status: 404 });
        }
        return NextResponse.json({ message: '삭제 완료' });
    } catch (error) {
        console.error('[DELETE /api/as/:id]', error);
