...
export async function PUT(
  ...
  try {
    ...
    if (isAdmin) {
        // 관리자는 현재 대기중인 모든 단계를 처리할 수 있음
        if (currentStepIndex === -1) {
             return NextResponse.json({ success: false, error: '이미 모든 결재가 완료되었습니다.' }, { status: 400 });
        }
        currentStep = approval.approvalLine[currentStepIndex];
    } else {
        currentStep = approval.approvalLine[currentStepIndex];
...
export async function DELETE(
    ...
    // 참고: 실제 인증 로직에서는 세션/토큰에서 사용자 ID를 가져와야 합니다.
    // 여기서는 예시로 요청 본문에서 requesterId를 받지만, 실제로는 보안에 취약합니다.
    const { requesterId } = await request.json(); // 데모용. 실제로는 인증된 사용자 ID 사용.
    ...
    try {
        ...
        // 결재 건 삭제 대신 상태를 'cancelled'로 변경하거나, 실제 삭제할 수 있습니다.
        // 여기서는 실제 배열에서 삭제하는 것으로 구현합니다.
